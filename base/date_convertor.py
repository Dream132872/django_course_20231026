...
class DateConvertor:
    ...
    def is_valid(date: str):
        if get_language() == "fa":
            return re.match("^[1-4]\d{3}\/((0[1-6]\/((3[0-1])|([1-2][0-9])|(0[1-9])))|((1[0-2]|(0[7-9]))\/(30|([1-2][0-9])|(0[1-9]))))$", date)

        return re.match("^\d{4}[\-\/\s]?((((0[13578])|(1[02]))[\-\/\s]?(([0-2][0-9])|(3[01])))|(((0[469])|(11))[\-\/\s]?(([0-2][0-9])|(30)))|(02[\-\/\s]?[0-2][0-9]))$", date)
    ...
    def to_current_time_zone_datetime(date: str):
        try:
            ...
            if get_language() == "fa":
                return jdatetime.datetime(year=int(year), month=int(month), day=int(day)).togregorian().replace(hour=12, minute=0, second=0)

            return datetime.datetime(year=int(year), month=int(month), day=int(day)).replace(hour=12, minute=0, second=0)
